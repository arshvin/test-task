...
const (
	appPackagePath = "./cmd/test-webapp"
)
...
func BuildMainApp(ctx context.Context) error {
	...
	return build(envs, path.Base(appPackagePath), appPackagePath)
}
...
func Install(targetFolder string) error {
	...
	return os.Rename(path.Base(appPackagePath), fmt.Sprintf("%s/%s", targetFolder, path.Base(appPackagePath)))
}

// Clean up after yourself
func Clean() {
	fmt.Printf("Cleaning of %s...\n", path.Base(appPackagePath))
	os.RemoveAll(path.Base(appPackagePath))
}
